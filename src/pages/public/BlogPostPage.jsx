...
export default function BlogPostPage() {
  ...
  if (loading) return <PageSpinner label="Cargando artículo" />;
  if (!post) {
    return (
  ...
  return (
    <article className="container-fluid max-w-4xl py-10 sm:py-14">
      ...
      >
        <ArrowLeft className="h-3.5 w-3.5" aria-hidden="true" /> Volver al blog
      </Link>

      <header className="mt-6">
        <Chip variant="accent" icon={<BookOpen className="h-3 w-3" aria-hidden="true" />}>
          Blog
        </Chip>
